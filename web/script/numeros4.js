...
function verifica_acta(total_candidatos, num_cand_lista)
{
    ...
    if (document.getElementById('txt_total_votos').value == '') {
        Swal.fire({
            icon: 'error',
            title: 'Error',
            text: 'Por favor ingrese el total de huellas o firmas.'
        });
        document.getElementById("aceptar").disabled = false;
        return;
    }
    if (document.getElementById('txt_votos_blanco').value == '') {
        Swal.fire({
            icon: 'error',
            title: 'Error',
            text: 'Por favor ingrese el total de votos en blanco.'
        });
        document.getElementById("aceptar").disabled = false;
        return;
    }
    if (document.getElementById('txt_votos_nulos').value == '') {
        Swal.fire({
            icon: 'error',
            title: 'Error',
            text: 'Por favor ingrese el total de votos nulos.'
        });
        document.getElementById("aceptar").disabled = false;
        return;
    }
    ...
    for (i = 1; i <= total_candidatos; i++)
    {
        ...
        if (parseInt(document.getElementById(i).value) > 350) {
            Swal.fire({
                icon: 'error',
                title: 'Error',
                text: 'Existe un valor mayor que 350.'
            });
            return;
        }
    }
    ...
    if (cuadrada) {
        if (select_cuadrada) {
            ...
            Swal.fire({
                icon: 'error',
                title: 'Error',
                text: 'Marque esta acta como cuadrada!'
            });
        ...
        if (!select_cuadrada) {//Si el check esta como no cuadrado
            ...
            Swal.fire({
                icon: 'error',
                title: 'Error',
                text: 'Marque esta acta como NO cuadrada!'
            });
            document.getElementById("aceptar").disabled = false;
        }
    }
