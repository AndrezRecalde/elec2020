...
public class candidatoDB {
    ...
    public int ContarCandidatosxDignidad(int dignidad) {
        int totalCandidatos = 0;
        Connection con = null;
        try {
            con = conexion.getConexion();
            String sql = "SELECT COUNT(c.fr_id_dignidad) as total_candidatos from candidato as c where c.fr_id_dignidad = " + dignidad;
            Statement st = con.createStatement();
            ResultSet rs = st.executeQuery(sql);
            while (rs.next()) {
                totalCandidatos = rs.getInt("total_candidatos");
            }
            rs.close();
            st.close();
            con.close();
